...
def load_data():
    data_to_load = [["Tiger Nixon", "System Architect", "Edinburgh", "5421", "2011/04/25", "$320,800"],
    ...
    for index, user in enumerate(data_to_load):
        ...
        last_name = user[0].split(" ")[1] or None
        position = user[1]
        location = user[2]
        extension = user[3]
        start_date = datetime.strptime(user[4], "%Y/%m/%d")
        salary = int(user[5].replace(",", "").replace("$", ""))
        user = UsersModel(first_name=first_name, last_name=last_name, position=position, location=location,
